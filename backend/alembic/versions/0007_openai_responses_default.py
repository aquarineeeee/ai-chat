...
def _official_openai_predicate(instances: sa.TableClause) -> sa.sql.elements.BooleanClauseList:
    return sa.and_(
        instances.c.preset_id == "openai",
        sa.or_(
            instances.c.base_url.is_(None),
            sa.func.trim(instances.c.base_url) == "",
            sa.func.lower(sa.func.trim(instances.c.base_url)).in_(_OFFICIAL_OPENAI_URLS),
        ),
    )
...
def upgrade() -> None:
    instances = sa.table(
        ...
        sa.column("default_adapter_id", sa.String()),
    )
    op.execute(
        instances.update()
        .where(_official_openai_predicate(instances))
